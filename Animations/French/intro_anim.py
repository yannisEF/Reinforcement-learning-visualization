...
class Introduction(Scene):
    ...
    def construct(self):
        # Authors
        splash = SplashScreen(title="\t\t\t\t\tSorbonne Université\n\nMaster Informatique : Projet ANDROIDE", titleScale=.5,
        ...
        title = Text("Visualisation du paysage de valeur").shift(3*UP).scale(.5)
        ...
        # Questions
        remark = TextItem("L'espace d'apprentissage du modèle est en dimension N.").shift(UP)
        question1 = TextItem("Comment visualiser la trajectoire du modèle ?").shift(UP)
        question2 = TextItem("Comment visualiser son environnement alentour ?")
        ...
        # Table of content
        part1 = TextItem("1 - Présentation des méthodes de visualisation").shift(1.5*UP)
        part2 = TextItem("2 - Structure des outils développés").shift(.5*UP)
        part3 = TextItem("3 - Démonstration de fonctionnement").shift(.5*DOWN)
        part4 = TextItem("4 - Développements futurs").shift(1.5*DOWN)
