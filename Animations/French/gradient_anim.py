...
class EtudeGradient(Scene):
    def construct(self):
        ...
        subtitle = Text("Méthode de l'étude de gradient").scale(.35).shift(2.5*UP)
        self.add(titre)
        self.play(FadeIn(subtitle))
        self.wait(2)

        # Intro
        text1 = TextItem(text="Suivre la descente de gradient d'un modèle").shift(UP)
        text2 = TextItem(text="Connaître les structures qu'il traverse")
        text3 = TextItem(text="Comparer les directions prises entre chaque pas").shift(DOWN)
        ...
        descr = Text("Environnement Swimmer, algorithme SAC de 250 à 10.000 pas").scale(.1725/1.5).shift(1.6*DOWN)
        ...
        conc1 = TextItem("On connaît les changements d'angle, les tâtonnements").shift(.5*UP)
        conc2 = TextItem("Apporte des informations sur la descente de gradient").shift(.5*DOWN)
