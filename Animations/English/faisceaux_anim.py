...
class MethodeFaisceaux(Scene):   
    ...
    def construct(self):
        # Chapter intro
        titre = SplashScreen(title="4 - Future developments", titleScale=.5, titleShift=2*UP)
        self.play(FadeIn(titre))
        self.wait(5)

        # Vitesse des calculs
        subTitle = Text("Processing speed").scale(.35).shift(1.5*UP)
        titre.add(subTitle)
        self.play(FadeIn(subTitle))
        self.wait(4)

        text1 = TextItem("Because of the generalisation problem, it is required to compute a sample's performance over several episodes.").shift(UP)
        text2 = TextItem("We seek a compromise between the desired precision and the available processing power")
        text3 = TextItem("Several solutions:").shift(DOWN)
        text4 = Group(TextItem("multi-core processing"),
                    TextItem("introducing checkpoints while sampling the lines").shift(.7*DOWN),
                    TextItem("compute the output over several times, increasing precision -> observe preliminary results").shift(2*.7*DOWN))
        ...
        subTitle2 = Text("The Beam method").scale(.35).shift(2.5*UP)
        titre.add(subTitle2)
        ...
        descr1 = TextItem("This method allows to compute a focused Spate around a given direction").shift(UP)
        descr2 = TextItem("We should get a better glimpse of the encountered environment\n during the gradient descent")
        descr3 = TextItem("Focused lines allow for less discontinuity in structural detection.").shift(1.1*DOWN)
