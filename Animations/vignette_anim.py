...
class VignettePortee(Scene):    
    ...
    def construct(self):
        # Title
        titre = SplashScreen(title="1 - Présentation des méthodes de visualisation", titleScale=.5, titleShift=3*UP)
        subtitle = Text("Méthode de la Vignette").scale(.35).shift(2.5*UP)
        self.add(titre)
        self.wait(4)
        ...
        # Intro
        text1 = TextItem(text="Obtenir un aperçu des alentours du modèle").shift(UP)
        text2 = TextItem(text="Connaître les structures auxquelles il est confronté")
        text3 = TextItem(text="Analyser la position relative d'autres points dans l'environnement observé").shift(DOWN)
        ...
        descr = Text("Exemple de Vignette, environnement Pendulum, algorithme SAC sur 5000 pas").scale(.1725).shift(3*RIGHT+offset+1.5*DOWN)
        ...
        conc1 = TextItem("Espace d'apprentissage en dimension N").shift(UP)
        conc2 = TextItem("Visualisation partielle de l'espace")
        conc3 = TextItem("Aperçu des structures alentours").shift(DOWN)
