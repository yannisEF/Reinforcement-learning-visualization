...
write_csv_average_mu = False #enregistrer en csv les scores moyens de l'acteur par itérations
...
if __name__ == "__main__":
    ...
    while total_steps < args.max_steps:
        ...
        #test forward avant d'exécuter CEM :
        #print(actor.forward()) #????? Quel type de paramètre pour forward ?...!!!!
        ...
        if step_cpt >= args.period:
            ...
            if(write_csv_average_mu):
                save_mu = open("average_mu_actor.csv", 'a')
    ...
    for _ in range(10000):
        #conversion de l'état dans le bon format :
        stateTorch = FloatTensor(np.array(state).reshape(-1)) #Variable(torch.from_numpy(np.array(state)).float().unsqueeze(0))
