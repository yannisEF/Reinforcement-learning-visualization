...
if __name__ == "__main__":
    ...
    for run in range(0,nb_runs_CEM):
        ...
        while total_steps < args.max_steps:
            ...
            print(es.elite)
            #save_file = 'data.csv'
            #np.savetxt(save_file, es.elite, delimiter=',')
            # Save Numpy array to csv
            #np.savetxt('array.csv', [es.elite], delimiter=',', fmt='%d')
            #print("size : ",es.elite.shape)
            """
            save_actual_elite = open("output.csv", 'a')
            save_actual_elite.write(str(es.elite.tolist())+"\r\n")#+ " : "+str(es.mu))
            save_actual_elite.close"""
            #np.savetxt('out.csv', es.elite, delimiter='\r\n')
            ...
            if step_cpt >= args.period:
                ...
                # evaluate elite actor over several runs. Memory is not filled
                # and steps are not counted
                actor.set_params(es.mu) #for vidéo frise, change to es.elite /es.mu
                f_mu, _ = evaluate2(actor, env, memory=None, n_episodes=args.n_eval,
                               render=args.render)
                ...
                string_rep = "./video_cem_"+str(1+run)+"_"+str(total_steps)
                env2 = gym.wrappers.Monitor(env2, string_rep,force=True)
                ...
                df.to_pickle(string_rep + "/log.pkl")
                ...
                #os.makedirs(args.output + "/mu_run_{}_{}_steps".format(str(1+run),total_steps),exist_ok=True)
                actor.set_params(es.elite)#mu
                actor.save_model(string_rep + "/cem_run_{}_{}_steps".format(str(1+run),total_steps), "actor_mu")
                """else:
