...
if __name__ == "__main__":
    ...
    TD3_END = 300000 # last iteration number of TD3
    CEM_END = 200000 # last iteration number of CEM
    NB_RUNS = 15 #number of runs of each algorithm. Here, small for short test of compilation, but usualy 52.
    ...
    for i in range(NB_RUNS):
        ...
        model.load_model("EXPLO/cem_explo","actor_cem_"+str(i)+"_0")
    ...
    for i in range(NB_RUNS):
        for j in range(CEM_STEP,CEM_END+CEM_STEP,CEM_STEP):
            ...
            model = Actor(state_dim, action_dim, max_action, args)
            model.load_model("EXPLO/cem_explo","actor_cem_"+str(i)+"_"+str(j))
            params = model.get_params()
            fit = evaluate(model, env, memory=memory, n_episodes=1, random=False, noise=None, render=False)
            global_data.append(params)
    ...
    for i in range(NB_RUNS):
        for j in range(TD3_STEP,TD3_END+TD3_STEP,TD3_STEP):
            ...
            model = Actor(state_dim, action_dim, max_action, args)
            model.load_model("EXPLO/td3_explo","actor_td3_"+str(i)+"_"+str(j))
            params = model.get_params()
            fit = evaluate(model, env, memory=memory, n_episodes=1, random=False, noise=None, render=False)
            global_data.append(params)
    ...
    plt.savefig("resGen_new"+str(NB_RUNS)+".png")
    plt.show()
