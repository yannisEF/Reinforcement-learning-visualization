...
#modifié pour retourner les valeurs de fitness au cours de l'épisode
...
if __name__ == "__main__":
    ...
    actors_directory = "resMemory" #dossier contenant les acteurs utilisés
    nb_run = 5
    steps = [5000,20000]#,30000,40000,50000]
    for step in steps :
        actors_to_compare = []
        for i in range(6,5+nb_run+1):
            actors_to_compare.append("actor_td3_buffer_"+str(i)+"_step_2_"+str(step))
        #for i in range(6,5+nb_run + 1):
        #    actors_to_compare.append("actor_td3_buffer_"+str(i)+"_step_1_"+str(step)) #nom des fichiers des acteurs (sans l'extension .pkl) à comparer (2 acteurs à donner)
        color_plot = ['r','r','r','r','r','b','b','b','b','b'] #on va tracer en bleu le score du premier acteur, en rouge celui du second
        #evaluate speed :
        plots = []
        for act_file in actors_to_compare:
            model = Actor(state_dim, action_dim, max_action, args)#on charge l'acteur
            model.load_model(actors_directory,act_file)
            fit, _, speeds = evaluate(model, env, memory=memory, n_episodes=1, random=False, noise=None, render=False)#on évalue sur un épisode
            plots.append(speeds[0]) #on ajoute aux tracés à faire

        # on créé la figure :
        plt.clf()
        plt.figure()
        lines = [0 for i in range(len(plots))]
        for i in range(len(plots)):
            lines[i], = plt.plot(list(range(len(plots[i]))), plots[i], label='run'+str(i+1))#, color = color_plot[i])
        plt.title("step "+str(step))#" buffer vide")
        plt.xlabel("temps")
        plt.ylabel("vitesse")
        plt.legend()#(lines[0], lines[nb_run+1]), ('buffer rempli', 'buffer vide'))
        plt.savefig("speed_compare_"+str(step)+"_filled_2.png")#sauvegarde sans un fichier (attention, changer le nom sinon ça écrase l'ancien)
    env.close()
