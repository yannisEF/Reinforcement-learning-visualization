...
save_average_mu_in_csv = False #enregistrer les différents mu average de l'actor dans un csv pendant l'apprentissage
    ...
    while total_steps < args.max_steps:
        ...
        if step_cpt >= args.period:
            ...
            if(save_average_mu_in_csv):
                ...
                save_mu.close
            string_rep = "./td3_explo"# "./actor_td3_"+str(1+run)+"_"+str(total_steps)
            if(render):
                ...
                string_rep = "./video_td3_"+str(1+run)+"_"+str(total_steps)
                ...
                while not done :
                    ...
                    action = agent.actor.forward(stateTorch).cpu().data.numpy().flatten()
                    #print(action)
            ...
            os.makedirs(string_rep,exist_ok=True)
            agent.actor.save_model(string_rep, "actor_td3_"+str(run)+"_"+str(total_steps))
            step_cpt = 0
            print(res)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_steps', default=150000, type=int)#1000000
    ...
    if args.mode == 'train':
        for run in range(41,151):
            agent.actor.load_model("cem_explo", "actor_cem_"+str(run)+"_0")#on charge les points de départ des CEM
            train(run,n_episodes=args.n_episodes, output=args.output, debug=args.debug, render=args.render)
