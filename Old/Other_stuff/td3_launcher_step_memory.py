    ...
    while total_steps < args.max_steps:
        ...
        if step_cpt >= args.period:
            ...
            if(save_average_mu_in_csv):
                ...
                save_mu = open("buffer_modif_td3.csv", 'a')
            ...
            agent.actor.save_model("resMemory", "actor_td3_buffer_"+str(run)+"_"+str(total_steps))
            step_cpt = 0
            print(res)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--nbRuns', default=4,type=int)
    ...
    for run in range(9,11):#range(1,1+args.nbRuns)
        ...
        for step_run in range(1,3):
            ...
            if args.mode == 'train':
                if(step_run==1):
                    print("STEP 1")
                    y,x = train(str(run)+"_step_1",n_episodes=args.n_episodes, output=args.output, debug=args.debug, render=False)#modif en brut
                    step1_runs_plot.append((x,y))
                else:
                    print("STEP 2")
                    y,x = train(str(run)+"_step_2",n_episodes=args.n_episodes, output=args.output, debug=args.debug, render=False)#modif en brut
                    step2_runs_plot.append((x,y))
            else:
                raise RuntimeError('undefined mode {}'.format(args.mode))

    env.close()
    plt.figure()
    for run in range(len(step1_runs_plot)):
        plt.plot(step1_runs_plot[run][0],step1_runs_plot[run][1], color ='b')
        plt.plot(step2_runs_plot[run][0],step2_runs_plot[run][1], color ='r')
    plt.show()
    plt.savefig("resMemory/result_9_10.png")
