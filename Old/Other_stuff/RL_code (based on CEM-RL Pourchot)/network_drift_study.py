...
if __name__ == "__main__":
    ...
    for i in range(1,NB_RUNS+1):
        ...
        for j in range(TD3_STEP,TD3_END+TD3_STEP,TD3_STEP):
            ...
            critic.load_model(directory, "critic"+filename)
            ...
            #fits.append(fit)
            td3_maxP.append(np.max(np.absolute(paramsA)))
            td3_maxQ.append(np.max(np.absolute(paramsC)))
            td3_meanabsP.append(np.mean(np.absolute(paramsA)))
            td3_meanabsQ.append(np.mean(np.absolute(paramsC)))
            td3_meanP.append(np.mean(paramsA))
            td3_meanQ.append(np.mean(paramsC))
            td3_steps.append(j)
        plt.figure()
        maxP, = plt.plot(td3_steps, td3_maxP, color="r")
        meanP, = plt.plot(td3_steps, td3_meanP, color="r", linestyle="dashed")
        meanabsP, = plt.plot(td3_steps, td3_meanabsP, color="purple")
        maxQ, = plt.plot(td3_steps, td3_maxQ, color="b")
        meanQ, = plt.plot(td3_steps, td3_meanQ, color="b", linestyle="dashed")
        meanabsQ, = plt.plot(td3_steps, td3_meanabsQ, color="orange")
        random = plt.axvline(x=start_steps, color = "g", linestyle="dashed")
        #fitness = plt.plot(td3_steps, fits, color = "g")
        plt.grid()
        plt.legend([maxP,meanP,maxQ,meanQ, random,meanabsP,meanabsQ], ["max abs actor", "mean actor", "max abs critic", "mean critic", "random policy end", "mean |critic|", "mean |actor|"])
        plt.show()
