...
def evaluate(actor, env, memory=None, n_episodes=1, random=False, noise=None, render=False, algo="cem",nb=0):
    """
    ...
    for _ in range(n_episodes):
        ...
        plt.figure()
        if(algo=="cem"):
            plt.plot(list(range(len(speed_episode))),speed_episode, color='b')
        elif(algo=="td3"):
            plt.plot(list(range(len(speed_episode))),speed_episode, color='r')
        plt.savefig("speeds_episode_"+str(algo)+"_"+str(nb)+".png")
...
if __name__ == "__main__":
    ...
    NB_RUNS = 1 #number of runs of each algorithm. Here, small for short test of compilation, but usualy 52.
    ...
    for i in range(NB_RUNS):
        ...
        fit, _, speeds = evaluate(model, env, memory=memory, n_episodes=1, random=False, noise=None, render=False,algo="cem",nb=0)
        nb_cem+=1
        fit, _, speeds = evaluate(model, env, memory=memory, n_episodes=1, random=False, noise=None, render=False,algo="td3",nb=0)
        nb_td3+=1
        for k in range(len(speeds)):
            init_speeds.append(speeds[k])
    ...
    for i in range(NB_RUNS):
        for j in range(CEM_STEP,CEM_END+CEM_STEP,CEM_STEP):
            ...
            params = model.get_params()
            fit, _, speeds = evaluate(model, env, memory=memory, n_episodes=1, random=False, noise=None, render=False,algo="cem",nb=nb_cem)
            print(len(speeds))
            nb_cem+=1
            for k in range(len(speeds)):
                cem_speeds.append(speeds[k])
    #loading TD3
    ...
    for i in range(NB_RUNS):
        for j in range(TD3_STEP,TD3_END+TD3_STEP,TD3_STEP):
            ...
            params = model.get_params()
            fit, _, speeds = evaluate(model, env, memory=memory, n_episodes=1, random=False, noise=None, render=False, algo="td3",nb=nb_td3)
            nb_td3+=1
            for k in range(len(speeds)):
                td3_speeds.append(speeds[k])
    ...
    """
    for i in range(NB_RUNS):
    	plt.plot(list(range(len(cem_speeds[i]))),cem_speeds[i], color='b')
    	plt.plot(list(range(len(td3_speeds[i]))),td3_speeds[i], color='r')
